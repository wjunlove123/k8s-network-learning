...
# 1.prep noCNI env
cat <<EOF | kind create cluster --name=cilium-native-routing-with-kubeproxy-ipsec --image=kindest/node:v1.23.4 --config=-
kind: Cluster
...
helm install cilium cilium/cilium --set k8sServiceHost=$controller_node --set k8sServicePort=6443 --version 1.12.0 --namespace kube-system --set debug.enabled=true --set debug.verbose=datapath --set monitorAggregation=none --set ipam.mode=cluster-pool --set cluster.name=cilium-native-routing-with-kubeproxy-ipsec --set kubeProxyReplacement=probe --set autoDirectNodeRoutes=true --set ipv4NativeRoutingCIDR=10.0.0.0/8 --set tunnel=disabled --set encryption.enabled=true --set encryption.type=ipsec

# 4. install necessary tools
for i in $(docker ps  -a --format "table {{.Names}}" | grep cilium-native-routing-with-kubeproxy-ipsec)
do 
    echo $i
